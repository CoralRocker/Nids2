...
fn main() {
    ...
    game::insert_object(&mut objects, naomi.clone());
    ...
    while !rl.window_should_close() {
        ...
        for obj in game::get_all_obj(&objects).iter() {
            game::update_object_in_list(&mut objects, obj.clone());
        }
        ...
        if pause {
            ...
            let (furn_button, furn_vec) = game::ds_rounded_button_centered(
                &mut d,
                rrect(scr_w as f32 * 0.25, scr_h - menu_height/2, scr_w/4, menu_height/2),
                Some("Select Furniture")
            );
            let (opt_button, opt_vec)  = game::ds_rounded_button_centered(
                &mut d,
                rrect(scr_w as f32 * 0.5, scr_h - menu_height/2, scr_w/4, menu_height/2),
                Some("Options")
            );

            let (exit_button, exit_vec) = game::ds_rounded_button_centered(
                &mut d,
